...
def test_01_extract_and():
    ...
    assert circuit.gates[0] == {"name": "Tof", "ctrl1": 0, "ctrl2": 1, "target": 2}
...
def test_02_extract_xor():
    ...
    assert circuit.gates[0] == {"name": "CNOT", "ctrl": 0, "target": 2}
    assert circuit.gates[1] == {"name": "CNOT", "ctrl": 1, "target": 2}
...
def test_03_extract_esop():
    ...
    assert circuit.gates[0] == {"name": "Tof", "ctrl1": 0, "ctrl2": 1, "target": 3}
    assert circuit.gates[1] == {"name": "Tof", "ctrl1": 0, "ctrl2": 2, "target": 3}
