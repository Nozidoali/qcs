            ...
            if op in assign_from:
                ...
                if map_or and op == "|":
                    ...
                    return LogicGate(
                        ...
                        [s1, s2],
                        assign_to,
                        {"p1": not p1, "p2": not p2, "p3": True},
                    )
                return LogicGate(
                    op, [s1, s2], assign_to, {"p1": p1, "p2": p2, "p3": False}
                )
...
class LogicNetwork:
    ...
    def to_verilog(self) -> str:
        ...
        verilog_str += (
            "  wire "
            + ", ".join(
                [k for k in self.gates.keys() if k not in self.inputs + self.outputs]
            )
            + ";\n"
        )
    ...
    def create_and(self, n: str, f1: str, f2: str) -> None:
        self.gates[n] = LogicGate(
            "&", [f1, f2], n, {"p1": False, "p2": False, "p3": False}
        )
    ...
    def simulate(self) -> int:
        if self.num_pis >= 5:
            raise NotImplementedError(
                "Simulation for more than 5 inputs is not implemented"
            )
        ...
        for _n, gate in self.gates.items():
            if gate.is_and:
                ...
                self._node_patterns[_n] = (
                    ~(_p1 & _p2) & mask if gate.data["p3"] else _p1 & _p2
                )
            elif gate.is_buf:
                ...
                _p2 = ~_p2 & mask if gate.data["p2"] else _p2
                self._node_patterns[_n] = (
                    (_p1 ^ _p2) & mask if gate.data["p3"] else ~(_p1 ^ _p2) & mask
                )
            else:
    ...
    def collect_nodes_in_topological_order(
        self, root: str, cut: list[str]
    ) -> list[str]:
        visited: set[str] = set()
        order: list[str] = []
...
def random_network(n: int, num_gates: int) -> LogicNetwork:
    network = LogicNetwork()
    for _ in range(n):
        network.create_pi(f"pi{_}")
    and_queue: set[tuple[int, int]] = set()
    xor_queue: queue.Queue = queue.Queue()
    while len(and_queue) < num_gates:
